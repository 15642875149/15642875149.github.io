...
function game() {
	...
	alert("电脑随机生成了一个1~32之间的数，请猜出这个数");
	var number = prompt("请输入想猜的数：");
	var secret = Random(1, 32)
	while (number != secret) {
		if (number > secret) {
			alert("你猜的数大了，请继续猜");
			number = prompt("请输入想猜的数：");
		} else {
			alert("你猜的数小了，请继续猜");
			number = prompt("请输入想猜的数");
		} 
	}
	alert("恭喜你，你猜对了，电脑生成的随机数是" + secret);
}
function linux(){
	window.location.href = './book/Linux/Linux程序设计.htm';
}
function index(){
